...
async function verifyGoogleIdTokenFallback(id_token, clientId) {
  ...
  if (payload.aud !== clientId) throw createError(401, `Google id_token ไม่ตรงกับ client ของเรา (aud=${payload.aud})`);
...
export const oauthGoogle = async (req, res, next) => {
  try {
    const raw = req.body || {};
    const id_token = typeof raw.id_token === 'string' ? raw.id_token.trim() : raw.id_token;
    if (!id_token) return next(createError(400, 'กรุณาส่ง id_token ของ Google'));
    ...
    return res.json({ message: 'เข้าสู่ระบบด้วย Google สำเร็จ', user, token });
  } catch (err) {
    return next(err);
  }
};
...
export const oauthFacebook = async (req, res, next) => {
  try {
    ...
    const meRes = await fetch(
      `https://graph.facebook.com/me?fields=${encodeURIComponent(fields)}&access_token=${encodeURIComponent(access_token)}`
    );
    const me = await meRes.json();

    if (me.error) {
      return next(createError(401, `Facebook token ไม่ถูกต้อง: ${me.error?.message || 'unknown error'}`));
    }

    if (!me.email) {
      return next(createError(400, 'บัญชี Facebook นี้ไม่มีอีเมล โปรดอนุญาตสิทธิ์อีเมลหรือสมัครด้วยวิธีอื่น'));
    }

    const picture =
      typeof me?.picture === 'object'
        ? me.picture?.data?.url
        : null;
    ...
    const token = issueJwtAndCookie(res, user);
    return res.json({ message: 'เข้าสู่ระบบด้วย Facebook สำเร็จ', user, token });
  } catch (err) {
    return next(err);
  }
};